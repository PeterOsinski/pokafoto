...
1. **Name+Size check (photos folder only):** Before any processing, check if a file with the same `original_name` AND `size_bytes` already exists in the target photos directory. If yes → status `skipped` with reason `duplicate_name_size`. The file is silently ignored — no upload, no processing.
2. **Content hash check (all files):** SHA-256 hash computed during upload. If an identical hash already exists → status `skipped` with reason `duplicate_content`. This catches renamed duplicates.
